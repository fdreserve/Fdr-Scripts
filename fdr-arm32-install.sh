...
#64 bit only
COIN_TGZ='https://github.com/fdreserve/fdr-blockchain/releases/download/V2.2.2/2021-02-25_fdreserve_v2221_arm32.zip'
...
function create_key() {
  ...
  if [[ -z "$COINKEY" ]]; then
    ...
    sleep 60
	echo -e "${RED}Waiting 60 sec to load the wallet{$NC}"
...
#apt-get update >/dev/null 2>&1
apt-get install -y unzip curl systemd >/dev/null 2>&1
if [ "$?" -gt "0" ];
  then
